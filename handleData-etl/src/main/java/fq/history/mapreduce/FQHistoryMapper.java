package fq.history.mapreduce;

import com.google.gson.Gson;
import com.google.gson.JsonSyntaxException;
import common.bean.Constance;
import common.util.CommonRule;
import fq.history.bean.FqHistoryLog;
import fq.history.bean.FqHistoryResultLog;
...
public class FQHistoryMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        FqHistoryLog log;
        try {
            log = gson.fromJson(line, FqHistoryLog.class);
        } catch (JsonSyntaxException e) {
            errorWriter.write("JsonSyntaxException : " + line);
            e.printStackTrace();
            return;
        }
        FqHistoryResultLog resultLog = getInsResultLog(log);
        outputKey.set(resultLog.getUser_id());
        outputValue.set(gson.toJson(resultLog));
        context.write(outputKey, outputValue);
    }

    private FqHistoryResultLog getInsResultLog(FqHistoryLog log) {
        FqHistoryResultLog resultLog = new FqHistoryResultLog();
        ...
        resultLog.setUser_id(String.valueOf(log.getUser_id()));
        resultLog.setScreen_name("");
        ...
        resultLog.setUser_type(log.getUser_type());
        resultLog.setVerified("");
        resultLog.setFollowers_count(String.valueOf(log.getFollower_count()));
        resultLog.setFriend_count(String.valueOf(log.getFollowing_count()));
        resultLog.setPost_count(String.valueOf(log.getTips_count()));
        ...
        resultLog.setAlg_result(log.getAlg_result());
        resultLog.setCheckins_count(String.valueOf(log.getCheckins_count()));
        ...
        resultLog.setLists_count(String.valueOf(log.getLists_count()));
        ...
        resultLog.setSend_state(log.getSend_state());
        resultLog.setTop_tips(log.getToptips());
        resultLog.setVisibility(log.getVisibility());
        resultLog.setFriends(String.valueOf(log.getFriends()));
