...
//    BlogSystemUser selectById(Integer userId);
    ...
    List<BlogSystemUserEntity> userLogin(@Param("username") String username, @Param("password") String password);
    ...
    @Query(value = "select e from BlogSystemUserEntity e where e.username = ?1 and e.password = ?2", nativeQuery = false)
    List<BlogSystemUserEntity> getUserByNameAndPassword(@Param("username") String username, @Param("password") String password);
}
